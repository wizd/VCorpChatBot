import { ContactInterface, MessageInterface, RoomInterface } from 'wechaty/impls';
...
import { getVCorpConfigByName } from '../db/sysconfig';
import { generateAuthCode } from '../db/helper';
import { VSubCode, addVSubCode, getVSubCode, getVSubCodeByTxId } from '../db/subcode';
...
export async function moneyTransferHandler(vcuser: UserProfile, message : MessageInterface, input: string, 
    contact: ContactInterface,
    room: RoomInterface | undefined, botWxId: string): Promise<void> {
    try
    {
        ...
        parseWeChatTransferXml(txtext).then(async (transfer) => {            
          console.log(transfer);
            ...
            if(transfer.transcationid === undefined || transfer.transcationid === null || transfer.transcationid === "") {
                console.log("This transfer has no transcationid");
                return;
            }

            if(room) {
                await message.say(`无法在群内进行订阅。请直接发消息给我并联系客服退款。`);
                return;
            }
            ...
            if(subcode2 && subcode2.code === code) {
                ...
                if(room){
                    await message.say("恭喜你！您的会员订阅已生成，有效期一年。订阅码已经发消息给您了，请注意查收。");

                    const payload = "恭喜你！您的会员订阅已生成，有效期一年。直接对话聊天机器人，输入下列文本即可兑换会员：\n\n兑换订阅码 " + code;
                    const toContact = await message.toContact();
                    await toContact.say(payload);
                }
                else {
                    await message.say("恭喜你！您的会员订阅已生成，有效期一年。直接对话聊天机器人，输入下列文本即可兑换会员：\n\n兑换订阅码 " + code);
                }
            }
            else {
