import { ContactInterface, MessageInterface, RoomInterface, WechatyInterface } from "wechaty/impls";
...
export const msgRootDispatcher = async (
  ...
  if(message.type() === 11) {
    await moneyTransferHandler(vcuser, message, input, contact, room, process.env.BOT_WXID!);
    return;
  }
...
`
  ...
  if(text.startsWith("兑换订阅码")) {
    await handleSubscriptionCode(vcuser, contact, message, text);
    return;
  }
