...
import { VCorpConfig, getVCorpConfigByName, saveVCorpConfig, vCorpConfig } from "../db/sysconfig";
...
async function getConfigValues(): Promise<string> {
  ...
  async function handleUserInput(input: string, changedById: ObjectId): Promise<string> {
    // 从用户输入中解析 configName 和 configValue
    const [command, configName, configValue] = input.trim().split(/\s+/);
  
    // 检查 configName 是否有效
    if (!["yearTextSubFee", "monthTextSubFee", "dayTextSubFee"].includes(configName)) {
